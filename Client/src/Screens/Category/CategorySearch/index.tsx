...
export const CategorySearch = () => {
  ...
  const filterIngredients = (value: string) => {
    ...
    resultArray.forEach((item) => {
      testArr = testArr.filter((i) =>
        i.ingredient.includes(item.toLowerCase())
      );
    });
  ...
  const filterData = () => {
    const recipName = tags
      .flatMap((tag) => tag.recipeList)
      .filter((i) => calculateMatchRatio(i.name, input) >= 50);
